...
const AgentList = ({ navigation, route }) => {
  ...
  const apiMembers = () => {
    const promise = new Promise((resolve, reject) => {
      Axios.get(Config.API_AGENT_LIST + `?page=${page}&keyword=${find}`,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    if (form.id != null && form.id != '') {
      if (redirect == 'list') {
        navigation.navigate('TransferStock', { agentId: form.id, agentTo: form.code+' - '+form.name })
      }
    }
  }, [form])
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <View style={styles.container}>
        <Header2 title='Pilih Agen' btn={() => navigation.goBack()} />
        <View
          style={{
            marginVertical: 10
          }}
