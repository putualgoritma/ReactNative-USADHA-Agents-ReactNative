...
import { add_to_package,add_to_cart } from './../../redux';
...
const Products = ({ navigation, route }) => {
  ...
  const cartReducer = useSelector((state) => route.params.dataType == 'Checkout' ? state.CartReducer.item : state.PackageReducer.item );
  ...
  const apiProducts = () => {
    ...
    let apiList = Config.API_LIST_PRODUCT_MEMBER_AGENT +'?agent_id='+ `${userReducer.id}&page=${page}&keyword=${find}`;//Config.API_LIST_PRODUCT_MEMBER
    if (dataType == 'Activasi' || dataType == 'Jaringan') {
      apiList = Config.API_LIST_PRODUCT_MEMBER_PACKAGE + `?activation_type_id=${activation_type_id}&page=${page}&keyword=${find}`;
    }
  ...
  const insertCart = (product) => {
    ...
    if (!penanda) {
      if (dataType == 'Upgrade' || dataType == 'Jaringan') {
        ...
        navigation.navigate('RO', { dataForm: dataForm, dataType: dataType });
