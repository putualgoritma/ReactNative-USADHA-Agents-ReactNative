...
const initialTrfStock = {
  item: [],
  count: 0,
};
const TrfStockReducer = (state = initialTrfStock, action) => {
  if (action.type === 'CHANGE_QTY_TRF_STOCK') {
    if (action.qtyItem < 0) {
      action.qtyItem = 0;
    }
    var status_exist = 0;
    state.item.map((cart) => {
      if (cart.id === action.id) {
        cart.qty = action.qtyItem;
        status_exist = 1;
      }
      
    });
    if(status_exist == 0){
      state.item[state.item.length] = action.dataItem;
      state.count = state.item.length;
    }
  } else if (action.type === 'CLEAR_CART_TRF_STOCK') {
    var itemArray = state.item;
    var i = 0;
    while (i < itemArray.length) {
        itemArray.splice(i, 1);
...
// const Token_api_one_signal = '';

// const TokenApiOneSignal = (state = Token_api_one_signal, action) => {
//   //kirim nilai initiallState ke state
//   if (action.type === 'TOKEN_API_ONE_SIGNAL') {
//     // console.log(action.token)
//     state = action.token
//     console.log('reducer TOken ONe Signal', state)
//   }
//   return state; //dan hasilkan nilai state saat reducer ini dipanggil di store,js
// };
